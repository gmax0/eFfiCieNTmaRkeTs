...
public class TradePublisher implements EventHandler<TradeEvent> {
    ...
    public BigDecimal calculateMaxActionableAmount(Trade t1, Trade t2) {
        ...
        BigDecimal availableCounterBalance = metadataAggregator.getBalance(t1.getExchange(),
                t1.getOrderActionType().equals(Order.OrderType.BID) ?
                        t1.getCurrencyPair().counter : t1.getCurrencyPair().base).getAvailable();
        BigDecimal availableBaseBalance =  metadataAggregator.getBalance(t2.getExchange(),
                t2.getOrderActionType().equals(Order.OrderType.BID) ?
                        t2.getCurrencyPair().counter : t2.getCurrencyPair().base).getAvailable();

        LOG.info("Available counter ({}) balance on {} : {}, Available base ({}) balance on {} : {}",
                t1.getCurrencyPair().counter, t1.getExchange(), availableCounterBalance,
                t2.getCurrencyPair().base, t2.getExchange(), availableBaseBalance);

        //TODO: implement counterMaximum and baseMaximum if you become an arbitrage whale
        Integer counterPriceScale = metadataAggregator.getPriceScale(t1.getExchange(), t1.getCurrencyPair());
        BigDecimal ex1OrderMinimum = metadataAggregator.getMinimumOrderAmount(t1.getExchange(), t1.getCurrencyPair());

        Integer basePriceScale = metadataAggregator.getPriceScale(t2.getExchange(), t2.getCurrencyPair());
        BigDecimal ex2OrderMinimum= metadataAggregator.getMinimumOrderAmount(t2.getExchange(), t2.getCurrencyPair());

        BigDecimal maxBaseAmount = t1.getAmount(); //t1.getAmount() && t2.getAmount() are assumed equivalent
        BigDecimal reqCounterToBuy = t1.getPrice().multiply(maxBaseAmount);
        reqCounterToBuy = reqCounterToBuy.add(reqCounterToBuy.multiply(t1.getFee())); //Assumes fees are in counter currency

        if (availableCounterBalance.compareTo(reqCounterToBuy) >= 0
                && availableBaseBalance.compareTo(maxBaseAmount) >= 0) {
            maxBaseAmount = maxBaseAmount.setScale(Integer.min(counterPriceScale, basePriceScale), BigDecimal.ROUND_DOWN);
            LOG.info("Base and counter balances are sufficient to buy and sell {} (scaled) of {}", maxBaseAmount, t1.getCurrencyPair());
        } else {
            LOG.info("Insufficient base or counter balances...calculating max actionable base amount...");
            BigDecimal maxBaseBuyable = (availableCounterBalance.subtract(availableCounterBalance.multiply(t1.getFee())))
            ...
            if (maxBaseBuyable.compareTo(ex1OrderMinimum) < 1) {
                LOG.info("Max base buyable is less than minimum order volume: {} on exchange {}", ex1OrderMinimum, t1.getExchange());
                return BigDecimal.ZERO;
            }
            if (maxBaseSellable.compareTo(ex2OrderMinimum) < 1) {
                LOG.info("Max base sellable is less than minimum order volume: {} on exchange {}", ex2OrderMinimum, t2.getExchange());
                return BigDecimal.ZERO;
            }
