...
public class KrakenExchangeStream extends AbstractExchangeStream {
  ...
  public KrakenExchangeStream(Configuration config, OrderBookBuffer orderBookBuffer) {
    if (config.getKrakenConfig().isEnabled()) {
      LOG.info("Initializing {}ExchangeStream.", exchangeName);
      this.isEnabled = true;

      ExchangeSpecification krakenSpec =
          new KrakenStreamingExchange().getDefaultExchangeSpecification();
